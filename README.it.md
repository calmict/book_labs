...
│           ├── start/         # file di partenza — condivisi, language-neutral
│           └── solution/      # soluzione testata — condivisa, language-neutral
...
condivisi tra le lingue (un \`deployment.yaml\` è identico in qualsiasi lingua).
Solo le consegne leggibili sono bilingui, con il suffisso
\`README.it.md\` / \`README.en.md\`.

**Edizioni.** Tutto ciò che è citato dalla 1ª edizione di un manuale vive in
\`edN/\` e viene congelato alla pubblicazione di quell'edizione. Un'edizione
successiva si aggiunge come nuova cartella \`edN/\`, lasciando intatta la
precedente, così ogni link di un libro già pubblicato continua a funzionare
per sempre.
...
Per aggiornare in seguito: \`git pull\`.

Ogni cartella esercizio ha un \`README\` con la consegna, gli obiettivi, i
prerequisiti e le istruzioni passo-passo. Apri \`start/\`, svolgi il lavoro e
confronta con \`solution/\` quando hai finito.
