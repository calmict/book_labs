...
# Chapter 9 — prepare (or tear down) the two web nodes for the ad-hoc lab.
#
#   bash nodes.sh up      build cap09-web1, cap09-web2 (sshd + python3 + sudo)
#   bash nodes.sh down     remove them
#
# You connect as the non-root 'deploy' user; it has passwordless sudo, so an
# ad-hoc command with -b escalates to root. Ports: web1 -> 2291, web2 -> 2292.
# The lab (and its ephemeral key) live in /tmp/cap09-lab.

NET=cap09-net
LAB=/tmp/cap09-lab
KEY="$LAB/key"
NODES=(cap09-web1:2291 cap09-web2:2292)
