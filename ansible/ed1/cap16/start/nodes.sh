...
# Chapter 16 — prepare (or tear down) the single node for the roles lab.
#
#   bash nodes.sh up      build cap16-web1 (sshd + python3 + sudo)
#   bash nodes.sh down     remove it
#
# You connect as the non-root 'deploy' user (passwordless sudo). Port: 2361.
# The lab (and its ephemeral key) live in /tmp/cap16-lab.
